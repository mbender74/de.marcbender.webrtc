...
import de.marcbender.webrtc.RecordedAudioToFileController;
...
public class PeerConnectionClient {
  ...
  public void createPeerConnection(final VideoSink localRender, final VideoSink remoteSink,
      final VideoCapturer videoCapturer,  SignalingParameters signalingParameters) {
  ...
  public void setRemoteAudioEnabled(final boolean enable) {
    executor.execute(() -> {
      ...
      if (getRemoteAudioTrack() != null) {
        getRemoteAudioTrack().setEnabled(enableRemoteAudio);
  ...
  public void setVideoLocalEnabled(final boolean enable) {
    executor.execute(() -> {
      renderVideo = enable;
      if (localVideoTrack != null) {
        localVideoTrack.setEnabled(renderVideo);
        ...
        if (queuedRemoteCandidates != null) {
          ...
          peerConnection.addIceCandidate(candidate, new AddIceObserver() {
            ...
            public void onAddSuccess() {
              Log.e(TAG, "Candidate " + candidate + " successfully added.");
            }
            @Override
            public void onAddFailure(String error) {
              Log.e(TAG, "Candidate " + candidate + " addition failed: " + error);
            }
          });
        }
      }
