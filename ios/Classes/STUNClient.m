...
- (void)requestPublicIPandPortWithUDPSocket:(GCDAsyncUdpSocket *)socket delegate:(id<STUNClientDelegate>)_delegate{
    ...
    // NSLog(@"STUN Binding Request=%@", stunRequest);
...
withFilterContext:(id)filterContext{
    ...
    // NSLog(@"STUN didReceiveData = %@", data);
    
    // Checks
    //
    if([data length] < 20){
        // NSLog(@"STUN didReceiveData. Length too short (not a STUN response). Please repeat request");
        return;
    }
    //
    //
    ...
    if(![[data subdataWithRange:NSMakeRange(4, 16)] isEqualToData:magicCookieAndTransactionID]){
        // NSLog(@"STUN magic cookie and/or transaction id check failed. Please repeat request");
        return;
    }
    //
    ...
    if(![[data subdataWithRange:NSMakeRange(0, 2)] isEqualToData:[NSData dataWithBytes:"\x01\x01" length:2]]){
        // NSLog(@"STUN a non-success STUN response received. Please repeat request");
        return;
    }
    ...
    // NSLog(@"STUN response Body Length = %d", responseBodyLength);
    ...
    if(xmaddr != nil){
        ...
        // NSLog(@"XOR-MAPPED-ADDRESS: %@", xAddr);
        // NSLog(@"xmport: %@", xPort);
        
    }else{
        // NSLog(@"STUN No XOR-MAPPED-ADDRESS found.");
    }
    ...
    // NSLog(@"=======STUN========");
    // NSLog(@"STUN IP: %@", ip);
    // NSLog(@"STUN Port: %@", port);
    // NSLog(@"STUN Port randomization: %d", [sock localPort] == [port intValue]);
    // NSLog(@"===================");
    // NSLog(@"\n");
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didSendDataWithTag:(long)tag{
    // NSLog(@"STUN didSendDataWithTag=%ld", tag);
}
